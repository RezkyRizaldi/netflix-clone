	...
	return (
		<section className="flex flex-col space-y-2 py-16 md:space-y-4 lg:h-[65vh] lg:justify-end lg:pb-12">
			<div className="absolute top-0 left-0 -z-10 h-[95vh] w-full">
				<Image src={`${baseUrl}${movie?.backdrop_path || movie?.poster_path}`} layout="fill" objectFit="cover" alt={`${movie?.title || movie?.name || movie?.original_name}`} />
			</div>
			...
			<div className="flex space-x-3">
				...
				<button type="button" className="flex items-center gap-x-2 rounded bg-gray-500/70 px-5 py-1.5 text-sm font-semibold transition-opacity hover:opacity-75 md:py-2.5 md:px-8 md:text-xl">
